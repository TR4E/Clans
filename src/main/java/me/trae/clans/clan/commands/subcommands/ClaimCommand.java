...
public class ClaimCommand extends ClanSubCommand implements EventContainer<TerritoryClaimEvent> {

    public ClaimCommand(final ClanCommand manager) {
        super(manager, "claim");
    }
    ...
    public void execute(final Player player, final Client client, final GlobalGamer globalGamer, final Clan clan, final String[] args) {
        ...
        if (!(this.canClaimLand(player, client, clan, chunk))) {
            return;
        }

        this.callEvent(new TerritoryClaimEvent(clan, player, client, chunk));
    }

    private boolean canClaimLand(final Player player, final Client client, final Clan clan, final Chunk chunk) {
        ...
        if (!(client.isAdministrating())) {
            if (clan.getTerritory().size() >= clan.getMaxClaims()) {
            ...
            for (final Player nearbyPlayer : UtilChunk.getChunkEntities(Player.class, chunk)) {
                ...
                if (nearbyPlayerClan == clan || nearbyPlayerClan.isAllianceByClan(clan) || clientManager.getClientByPlayer(player).isAdministrating()) {
                    continue;
                }
    ...
    @Override
    public Class<TerritoryClaimEvent> getClassOfEvent() {
        return TerritoryClaimEvent.class;
    }

    @Override
    public void onEvent(final TerritoryClaimEvent event) {
